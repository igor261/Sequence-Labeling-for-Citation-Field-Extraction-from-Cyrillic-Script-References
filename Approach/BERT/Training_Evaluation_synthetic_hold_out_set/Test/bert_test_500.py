...
tag_values = open("/local/users/ulede/BERT/tag_values_500","r",encoding="utf-8")
...
model = torch.load("/local/users/ulede/BERT/model_500.pth")
...
for test, test_labels in zip(test_text, test_label):
  for values in ["publisher","title","journal","author","volume","other","pages","year","number","address","booktitle","pagetotal"]:
    test = test.replace(f"<{values}>","").replace(f"</{values}>","")
  test_sentence = test
  tokenized_sentence = tokenizer.encode(test_sentence)
...
print(Counter(new_label_eval))



myfile = open("/local/users/ulede/BERT/" + "eval_500.txt", "w",encoding="utf-8")
