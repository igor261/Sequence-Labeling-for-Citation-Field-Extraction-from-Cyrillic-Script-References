...
path_test = ".\Sequence-Labeling-for-Reference-Parsing-of-Cyrillic-Script-Scholarly-Data\Real_annotated_data\labelled_text_per_paper"
onlyfiles = [f for f in listdir(path_test) if isfile(join(path_test, f))]
...
output_text = "EVALUATION ON REAL CITATION STRING DATA SET. 711 REFERENCES\n\n\n"
...
#path_input = "/local/users/ulede/BERT/test_set_synth/"
path_input = ".\Sequence-Labeling-for-Reference-Parsing-of-Cyrillic-Script-Scholarly-Data\Synthetic_data\test_set_synth"
...
for file in onlyfiles:
    file2 = open(os.path.join(path_input,file), encoding='utf-8', mode='r')
    text = file2.read()
    tokens = text.split()
    labels_text = []
    for token in tokens:
...
for test, test_labels in zip(test_text, test_label):
  for values in ["publisher","title","journal","author","volume","other","pages","year","number","address","pagetotal","booktitle"]:
    test = test.replace(f"<{values}>","").replace(f"</{values}>","")
  test_sentence = test
  tokenized_sentence = tokenizer.encode(test_sentence)
...
target_names = ['author','publisher','volume','other','number','pages','address','journal','title','year',"booktitle","pagetotal"]
...
myfile = open("/local/users/ulede/BERT/final_model/" + "eval_final_2.txt", "w",encoding="utf-8") #".\Sequence-Labeling-for-Reference-Parsing-of-Cyrillic-Script-Scholarly-Data\BERT\BERT_final_model\eval_final_2.txt"
